...
if plotType == "Threshold":
    h.SetMinimum(1400)
    h.SetMaximum(2200) # 500vcal * 50e/vcal
if plotType == "Noise":
    h.SetMinimum(0)
    h.SetMaximum(200)
...
for d in dirs:
    ...
    for key in f.Get(d).GetListOfKeys():
        ...
        val = [50*obj.GetBinContent(i+1, j+1) for j in range(row) for i in range(col)] # 50e/vcal
        datapool[rocKey] = val
...
title = "FPix {0} Distribution - Run_{1}".format(plotType,str(run))
...
c.SaveAs(os.path.join(run_dir,"{0}.png".format('-'.join(["FFpix", plotType,str(run)]))))
